...
module('Unit | Utility | bugsnag configuration', () => {
	test('passes initialized values to bugsnag instance', (assert) => {
	...
	test('sets default a default for notifyReleaseStages', (assert) => {
		const configuration = new BugsnagConfiguration({
			apiKey: 'UALSCA319'
		}, 'insomnia').config;
	...
	test('does not set any values if there is a configuration problem', (assert) => {
